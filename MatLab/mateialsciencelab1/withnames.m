...
for i = 1:numClasses
    ...
    if ~isempty(data.(sanitized_class).T)
        ...
        Names_sorted = data.(sanitized_class).Names(sortIdx);
        
        scatter(data.(sanitized_class).T, data.(sanitized_class).DeltaRhoK, 50, 'filled');
        hold on;
        plot(T_sorted, DeltaRhoK_sorted, '-o', 'LineWidth', 1.5);
        
        % Add labels to points
        for j = 1:length(data.(sanitized_class).T)
            text(data.(sanitized_class).T(j), data.(sanitized_class).DeltaRhoK(j), Names_sorted{j}, 'FontSize', 8, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right');
        end
    end
    title(classes{i});
